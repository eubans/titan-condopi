...
class Pages extends MY_Controller {
    ...
    public function index(){
        ...
        $count_table =  $this->Common_model->count_table("Page",$where);
        ...
        $this->data["table_data"] = $this->Common_model->get_result("Page",$where,$offset,$per_page);
    ...
    public function create() {
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('Type_Page', 'Type Page', 'required');
            if ($this->form_validation->run() == TRUE) {
                $colums = $this->db->list_fields('Page');
                ...
                $id = $this->Common_model->add("Page",$data_insert);  
    ...
    public function delete($id = 0){
    	$this->Common_model->delete("Page",array("ID" => $id));
    ...
    public function edit($id = null){
    	...
    	$record = $this->Common_model->get_record("Page",array("ID" => $id));
    	if($record == null)
    		redirect(backend_url($this->base_controller));
        if($this->input->post()){
            $this->form_validation->set_rules('Key_Identify', 'Slug', 'required');
            ...
            if ($this->form_validation->run() == TRUE){
                ...
                $this->Common_model->update("Page",$data_update,array("ID" =>$record["ID"]));  
